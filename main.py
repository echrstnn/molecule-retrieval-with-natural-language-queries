from fast_dataloader import GraphTextDataset, GraphDataset, TextDataset
...
val_dataset = GraphTextDataset("./data/", gt, split="val", tokenizer=tokenizer)
train_dataset = GraphTextDataset("./data/", gt, split="train", tokenizer=tokenizer)
...
model = Model(
    model_name=model_name, num_node_features=300, nout=768, nhid=300, graph_hidden_channels=300
).to(device)
    ...
    for batch in tqdm(train_loader):
        ...
        with torch.autocast("cuda", dtype=torch.float16):
            x_graph, x_text = model(
                graph_batch.to(device, non_blocking=True),
...
checkpoint = torch.load("model9.pt")
...
test_cids_dataset = GraphDataset("./data/", gt, split="test_cids")
